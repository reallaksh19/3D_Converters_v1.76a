  ...
  return JSON.stringify({
    elements: {
      ...
      line_label: {
        ...
        label_prefix: 'LINENO',
        label_width: 1,
        // Must be empty (not '0') for mode:'auto' to actually generate
        // sequential labels - inputxml_to_cii2019.py's line_label branch
        // is `elif mode == "auto" and not default_line`, so a truthy
        // default_line short-circuits auto mode back to a static value.
        default_line: '',
        line_labels: [],
      },
    },
...
const CONVERTER_DEFS = {
  ...
  stagedjson_to_inputxml: {
    ...
    secondaryLabel: '',
    secondaryAccept: '',
    description: 'Convert staged hierarchy JSON to CAESAR II InputXML with explicit source authority and a diagnostics sidecar.',
    defaults: {
      inputxmlBookmarkJson: '{}',
      nodeStart: 10,
      nodeStep: 10,
      temperature1: 50.0,
      wallThickness: 0.01,
    ...
    fields: [
      { key: 'inputxmlBookmarkJson', label: 'Defaults Bookmark JSON', type: 'json-popup' },
      { key: 'nodeStart', label: 'Node Start', type: 'number', step: '1' },
      { key: 'nodeStep', label: 'Node Step', type: 'number', step: '1' },
      ...
      { key: 'materialNum', label: 'Material Number', type: 'number', step: '1' },
      { key: 'materialName', label: 'Material Name', type: 'text' },
      { key: 'verticalAxis', label: 'Vertical Axis (PDMS Up ->)', type: 'select', options: ['Y', 'Z'] },
      { key: 'inferOdFromNominalBore', label: 'Compatibility: infer OD from nominal bore', type: 'checkbox' },
      { key: 'autoAnchors', label: 'Infer compatibility anchors', type: 'checkbox' },
      { key: 'disableSupports', label: 'Disable support -> restraint mapping', type: 'checkbox' },
      { key: 'supportConfigJson', label: 'Support Restraint Config JSON (overrides)', type: 'json-popup' },
  ...
  seljson_to_inputxml: {
    id: 'seljson_to_inputxml',
    label: 'SelJson -> InputXML',
    primaryAccept: '.json,.JSON',
    primaryLabel: '3DV Conv JSON Input',
    secondaryLabel: '',
    secondaryAccept: '',
    description: 'Convert 3DV Conv JSON selections (rvm-converter-stage/v1) to Custom InputXML while preserving Position records.',
    defaults: {
      nodeStart: 10,
      nodeStep: 10,
      wallThicknessMm: 0,
      ...
      fluidDensity: '',
      fallbackBranchName: '',
      dropShortElementLengthNodes: false,
      includeNonRouteGeometry: false,
    },
    fields: [
      { key: 'nodeStart', label: 'Node Start', type: 'number', step: '1' },
      { key: 'nodeStep', label: 'Node Step', type: 'number', step: '1' },
      { key: 'fallbackBranchName', label: 'Fallback Branch Name', type: 'text' },
      { key: 'wallThicknessMm', label: 'Wall Thickness (mm)', type: 'number', step: '0.001' },
      { key: 'defaultPipingClass', label: 'Default Piping Class', type: 'text' },
      { key: 'defaultRating', label: 'Default Rating', type: 'text' },
      { key: 'p1', label: 'Pressure 1', type: 'number', step: '0.1' },
      { key: 't1', label: 'Temperature 1', type: 'number', step: '0.1' },
      { key: 'fluidDensity', label: 'Fluid Density', type: 'number', step: '0.1' },
      { key: 'dropShortElementLengthNodes', label: 'Drop ElementLengthMm <= 6 mm nodes', type: 'checkbox' },
      { key: 'includeNonRouteGeometry', label: 'Include Box/FacetGroup diagnostic nodes', type: 'checkbox' },
    ],
  },
  seljson_to_xml: {
    id: 'seljson_to_xml',
    label: 'SelJson -> XML',
    primaryAccept: '.json,.JSON',
    primaryLabel: '3DV SelJson Input',
    secondaryLabel: '',
    secondaryAccept: '',
    description: 'Convert 3DV selection JSON directly to PipeStressExport XML through Custom InputXML, without topology audit/build.',
    defaults: {
      nodeStart: 10,
      nodeStep: 10,
      wallThicknessMm: 0,
  ...
  inputxml_to_xml: {
    ...
    primaryLabel: 'InputXML Input',
    secondaryLabel: '',
    secondaryAccept: '',
    description: 'Convert Custom InputXML or CAESAR InputXML into PSI116-style XML for XML -> CII (2019).',
    defaults: {
      source: 'InputXML -> XML',
      purpose: 'InputXML bridge conversion',
      titleLine: 'InputXML converted XML',
      nodeStart: 10,
      nodeStep: 10,
      defaultDiameter: 100,
  ...
  inputxml_to_cii2019: {
    ...
    fields: [
      { key: 'inferReducerAngleFromGeometry', label: 'Infer Reducer Angle From Geometry', type: 'checkbox' },
      { key: 'defaultDiameter', label: 'Fallback Diameter if DIAMETER Missing', type: 'number', step: '0.001' },
      { key: 'defaultWallThickness', label: 'Fallback Wall Thickness if Missing', type: 'number', step: '0.001' },
      { key: 'defaultInsulationThickness', label: 'Fallback Insulation Thickness if Missing', type: 'number', step: '0.001' },
      { key: 'defaultCorrosionAllowance', label: 'Fallback Corrosion Allowance if Missing', type: 'number', step: '0.001' },
      { key: 'defaultTemperature1', label: 'Fallback Temperature1 if Missing', type: 'number', step: '0.01' },
      { key: 'defaultTemperature2', label: 'Fallback Temperature2 if Missing', type: 'number', step: '0.01' },
      { key: 'defaultTemperature3', label: 'Fallback Temperature3 if Missing', type: 'number', step: '0.01' },
      { key: 'defaultReducerAngle', label: 'Default Reducer Angle', type: 'number', step: '0.01' },
      { key: 'coordReconstructionTolerance', label: 'Coord Reconstruction Tolerance (mm)', type: 'number', step: '0.001' },
      { key: 'headerDateTime', label: 'Header DateTime', type: 'text' },
      { key: 'headerSource', label: 'Header Source', type: 'text' },
...
import { run as selJsonToInputXmlRun } from './converters/seljson-to-inputxml.js';
import { run as selJsonToXmlRun } from './converters/seljson-to-xml.js';
import { run as stagedJsonToCsvRun } from './converters/stagedjson-to-csv.js';
import { run as inputXmlToXmlRun } from './converters/inputxml-to-xml.js';
